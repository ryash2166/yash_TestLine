...
export default function Quiz() {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4'>
      {showConfetti && <Confetti width={width} height={height} recycle={false} numberOfPieces={1200} />}
        ...
        >
          ...
          <div className='grid md:grid-cols-2 gap-4'>
            {questions[currentQuestion]?.options?.map((option, index) => {
              ...
              const showCorrect = isAnswerSelected && isCorrect;
              ...
              return (
                <motion.button
                  ...
                  className={`p-4 rounded-xl text-lg font-medium transition-all duration-300 relative overflow-hidden ${
                    isSelected
                      ? isCorrect
                        ? 'bg-green-500 shadow-lg'
                        : 'bg-red-500 shadow-lg'
                ...
                >
                  {showCorrect && (
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="absolute inset-0 border-4 border-green-500 rounded-xl"
                      style={{ boxShadow: '0 0 20px rgba(72,187,120,0.5)' }}
                    />
                  )}
